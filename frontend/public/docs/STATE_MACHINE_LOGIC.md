...
```
MA斜率 = (当前MA值 - N天前的MA值) / N天前的MA值 × 100%
```
...
- **MA50斜率 > 0**：中期动能充沛，趋势质量高
- **MA50斜率 < 0**：典型的空头排列，反弹仅视为压力位修复而非反转
...
```
趋势多 = (BTC价格 > MA50 > MA200) AND (MA50斜率 > 0) AND (MA200斜率 > 0)
趋势空 = (BTC价格 < MA50 < MA200) AND (MA50斜率 < 0) AND (MA200斜率 < 0)
```
